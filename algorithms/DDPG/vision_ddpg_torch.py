...
from .vision_network_torch import *
import time


class Vision_Agent(object):
    ...
    def learn(self):
        ...
        target_actions = self.target_actor.forward(new_state, new_depth,new_cs)/10
        critic_value_ = self.target_critic.forward(new_state, new_depth,new_cs, target_actions)
        critic_value = self.critic.forward(state, depth,cs, action)
        ...
        mu = self.actor.forward(state, depth,cs)/10
        self.actor.train()
        actor_loss = -self.critic.forward(state, depth, cs, mu)
