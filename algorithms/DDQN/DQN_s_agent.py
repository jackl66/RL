from ..replay_buffer.twoD_Buffer import  twoD_ReplayBuffer as ReplayBuffer
from .DQN_s_network import *
import math


class DQN_s_agent(object):
    ...
    def choose_action(self, observation, ratio):
        ...
        if rate > np.random.rand():
            return np.random.randint(27)  # explore
        else:
            with T.no_grad():
                ...
                action = self.Qnet.forward(observation, depth, ratio).to(self.device)  # exploit
    ...
    def learn(self, ratio):
        ...
        current_Q1 = self.Qnet.forward(state, depth, ratio)

        target_Q = self.target_Qnet.forward(new_state, new_depth, ratio)

        next_q = self.Qnet.forward(new_state, new_depth, ratio)
        ...
        for i in range(self.batch_size):
            ...
            temp[i][action[i]] = TD_target.to(self.device)
