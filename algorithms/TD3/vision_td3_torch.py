...
from .vision_netd3_torch import *

class Vision_td3_Agent(object):
    ...
    def choose_action(self, observation):
        ...
        mu = self.actor.forward(observation, depth,cs).to(self.device)
    ...
    def learn(self):
        ...
        # unpack new element depth as well
        ...
        with T.no_grad():
            ...
            next_action = (self.target_actor.forward(new_state, new_depth, new_cs) + noise).clamp(-1, 1) /10

            # Compute the target Q value
            target_Q1, target_Q2 = self.target_critic(new_state, new_depth, new_cs, next_action)
        ...
        current_Q1, current_Q2 = self.critic(state, depth, cs, action)
        # Compute critic loss
        ...
        if self.count % self.update_freq == 0:
            ...
            # Compute actor lose
            actor_loss = -self.critic.Q1(state, depth, cs, self.actor.forward(state, depth, cs)/10)
